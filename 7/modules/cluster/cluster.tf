...
resource "null_resource" "config_vds" {
  ...
  provisioner "local-exec" {
    command = "ansible-playbook -vvv --extra-vars \"mgmtPortgroup=${var.mgmtPortgroup} vcenterHost=${var.vCenterHost} datacenterName=${var.dcName} vdsName=${var.vdsName} vlanID=${var.mgmtVlan}\" configVDS.7.yaml"
    environment = {
...
resource "null_resource" "config_vmk0_services" {
  ...
  provisioner "local-exec" {
    command = "ansible-playbook --extra-vars \"mgmtPortgroup=${var.mgmtPortgroup} vcenterHost=${var.vCenterHost} vdsName=${var.vdsName}\" configVMK0.7.yaml"
    environment = {
      nestedESXiJSONEncoded = var.nestedESXiJSONEncoded
    }
  }
...
#  provisioner "local-exec" {
#    command = "ansible-playbook TPMandVSANPrep.7.yaml"
#  }
#}
